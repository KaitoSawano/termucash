...
#if defined(NDEBUG)
# error "Termucash cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Termucash Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        if (fLimitFree && nModifiedFees < GetTermucashMinRelayFee(tx, nSize, !fLimitFree))
        {
...
bool CheckTxInputs(const CChainParams& params, const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight)
{
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            if (coins->IsCoinBase()) {
                // Termucash: Switch maturity at depth 145,000
...
void ThreadScriptCheck() {
    RenameThread("termucash-scriptch");
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // Termucash: BIP30 has been active since inception
    ...
    // Termucash: BIP16 has been enabled since inception
    ...
    CAmount blockReward = nFees + GetTermucashBlockSubsidy(pindex->nHeight, chainparams.GetConsensus(pindex->nHeight), hashPrevBlock);
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // Termucash: Disallow AuxPow blocks before it is activated.
    ...
    // Termucash: Version 2 enforcement was never used
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const CBlockIndex* pindexPrev)
{
    ...
    // Termucash: We probably want to disable this
    int nLockTimeFlags = 0;
