...
/*
 * Termucash: Scaled to 1/10th of the recommended transaction fee to make RBF
...
/*
 * Termucash: Creating change outputs at exactly the dustlimit is counter-
...
public:
    ...
    /**
     * Termucash: Get a fee targeting a specific transaction speed.
     */
    CAmount GetTermucashPriorityFee(const CMutableTransaction& tx, unsigned int nTxBytes, FeeRatePreset nSpeed);
    /**
     * Termucash: Get a fee targeting a specific transaction speed.
     */
    static CAmount GetTermucashPriorityFee(const CMutableTransaction& tx, unsigned int nTxBytes, FeeRatePreset nSpeed, CAmount targetFee);
    /**
    ...
    /**
     * Return the minimum required fee taking into account the
     * floating relay fee and user set minimum transaction fee, but not the Termucash dust fee.
     */
