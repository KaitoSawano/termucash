...
const char * const BITCOIN_CONF_FILENAME = "termucash.conf";
const char * const BITCOIN_PID_FILENAME = "termucashd.pid";
...
#else
    const char* pszModule = "termucash";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Termucash";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Termucash";
#else
    // Unix
    return pathRet / ".termucash";
#endif
#endif
}
