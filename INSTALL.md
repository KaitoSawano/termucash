# Installing Termucash Core

### Pre-compiled binaries

The easiest way to install the latest version of the Termucash Core software is
by to download the latest precompiled binaries for your platform from the
[release page](https://github.com/termucash/termucash/releases). Currently,
...
It is possible to run Termucash Core on other systems and lower versions when
...
[`gitian.sigs` repository on GitHub](https://github.com/termucash/gitian.sigs)
...
It is possible to build your own copy of Termucash Core with the exact, tested,
dependencies, as used for the binary releases, by using the
[depends system](depends/description.md). Please refer to the
[depends README](depends/README.md) for instructions to build Termucash using
these dependencies.

### Compiling using system-provided libraries

  The following are developer notes on how to build Termucash on your native
  ...
  Outdated libraries can render the entire system, including Termucash Core, vulnerable.
...
Run with the `-testnet` option to run with "play termucashs" on the test network, if you
...
Termucash Core is a multithreaded application, and deadlocks or other multithreading bugs
