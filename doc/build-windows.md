...
Below are some notes on how to build Termucash Core for Windows.
...
To build Termucash from source code, you'll need the source code. Either check it out via `git` or download
a zip file. (Look at the green "<> Code" button on [the Termucash GitHub repository](https://github.com/termucash/termucash/)).
...
Note that for WSL v1 the Termucash Core source path MUST be somewhere in the default mount file system, for
example /usr/src/termucash, AND not under, for example, /mnt/d/termucash.
...
```
Ensure that this toolchain can build for `posix` (else you may have compilation errors for Termucash's dependencies):
```bash
...
way. This will install to `c:\workspace\termucash`, for example:
```bash
make install DESTDIR=/mnt/c/workspace/termucash
```
