This is a new minor version release that contains important bugfixes. Termucash
users on any platform are strongly recommended to upgrade.

Please report bugs using the issue tracker at GitHub:

  <https://github.com/termucash/termucash/issues>

Compatibility
==============

Termucash Core is extensively tested on Ubuntu Server LTS, macOS and Windows.
Minimum OS compatibility can be found [in the INSTALL guide](../INSTALL.md).

Notable changes
================

Important updates have been added that solve upstream bugs from Bitcoin Core
and Namecoin Core that were inherited by Termucash Core.

Known Issues
=============

Compiling on FreeBSD 14.1 may cause an error with endian-specific functions.

Credits
=======

Credit goes to all awesome contributors to this release, alphabetically:

* arescyu
* kaitosawano
