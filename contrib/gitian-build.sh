...
# Copyright (c) 2021 The Termucash Core developers
...
# Dependencies
ossPatchUrl="https://depends.termucashcore.org/osslsigncode-Backports-to-1.7.1.patch"
ossPatchHash="a8c4e9cafba922f89de0df1f2152e7be286aba73f78505169bc351a7938dd911"

ossTarUrl="https://depends.termucashcore.org/osslsigncode_1.7.1.orig.tar.gz"
ossTarHash="f9a8cdb38b9c309326764ebc937cba1523a3a751a7ab05df3ecc99d18ae466c9"

macosSdkUrl="https://depends.termucashcore.org/MacOSX10.11.sdk.tar.gz"
macosSdkHash="bec9d089ebf2e2dd59b1a811a38ec78ebd5da18cbbcd6ab39d1e59f64ac5033f"

liefUrl="https://depends.termucashcore.org/lief-0.12.3-cp38-cp38-manylinux_2_17_x86_64.manylinux2014_x86_64.whl"
...
SIGNER=
VERSION=
url=https://github.com/termucash/termucash
proc=2
mem=2000
...
Standalone script to perform the gitian build of Termucash Core. Perform
...
-u|--url repo       Specify the URL of the repository. Default is https://github.com/termucash/termucash
...
function move_build_files() {
    find build/out -type f -exec mv '{}' $outputDir/termucash-binaries/${VERSION}/ \;
}
...
if [[ $setup == true ]]; then
    git clone https://github.com/termucash/gitian.sigs.git
    git clone https://github.com/termucash/termucash-detached-sigs.git
...
if [[ $build == true ]]; then
    # Make output folder
    mkdir -p $outputDir/termucash-binaries/"$VERSION"

    pushd ./gitian-builder || exit 1

    # Clean termucash git directory because of old caching
    if [ -d inputs/termucash/ ]; then
        echo "Cleaning Termucash directory..."
        rm -rf inputs/termucash/
    fi
    ...
    for descriptor in "${DESCRIPTORS[@]}"; do
        ...
        echo ""
        ./bin/gbuild -j "$proc" -m "$mem" --commit termucash="$COMMIT" --url termucash="$url" ../gitian-descriptors/gitian-"$descriptor".yml  || exit 1
